...
const AdminDashboard = () => {
      ...
      <div className="min-h-[90vh] pt-5 flex flex-col flex-wrap gap-10 text-white">
        ...
        <div className="mx-[10%] w-[80%] self-center flex flex-col items-center justify-center gap-10 mb-10">
          ...
          <div className="bg-black border border-gray-700 rounded-lg overflow-hidden">
            <table className="w-full bg-black admin-table">
              <thead className="bg-gray-800">
                <tr>
                  <th className="text-white border-b border-gray-700 p-3 text-left">S No.</th>
                  <th className="text-white border-b border-gray-700 p-3 text-left">Course Title</th>
                  <th className="text-white border-b border-gray-700 p-3 text-left">Course Category</th>
                  <th className="text-white border-b border-gray-700 p-3 text-left">Instructor</th>
                  <th className="text-white border-b border-gray-700 p-3 text-left">Total Lectures</th>
                  <th className="text-white border-b border-gray-700 p-3 text-left">Course Description</th>
                  <th className="text-white border-b border-gray-700 p-3 text-left">Actions</th>
                </tr>
              </thead>
  ...
  {myCourses?.map((element, index) => {
    return (
      <tr key={element?._id} className="border-b border-gray-700 hover:bg-gray-900 bg-black">
        <td className="text-white p-3 bg-black">{index + 1}</td>
        <td className="text-white p-3 bg-black">
          <textarea
            readOnly
            className="w-40 h-auto bg-black resize-none text-white border-none outline-none"
            value={element?.title}
          ></textarea>
        </td>
        <td className="text-white p-3 bg-black">{element?.category}</td>
        <td className="text-white p-3 bg-black">{element?.createdBy}</td>
        <td className="text-white p-3 bg-black">{element?.numberOfLectures}</td>
        <td className="max-w-28 overflow-hidden text-ellipsis whitespace-nowrap text-white p-3 bg-black">
          <textarea
            readOnly
            className="w-80 h-auto bg-black resize-none text-white border-none outline-none"
            value={element?.description}
          ></textarea>
        </td>

        <td className="flex items-center gap-4 p-3 bg-black">
          <button
            onClick={() =>
              navigate("/course/create", {
                state: {
