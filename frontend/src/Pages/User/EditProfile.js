...
const EditProfile = () => {
  ...
  // Initialize data when userData is available
  useEffect(() => {
    if (userData) {
      setData({
      ...
      if (userData.avatar?.secure_url) {
        setImagePreview(userData.avatar.secure_url);
      }
    }
  }, [userData]);
  ...
  const getImage = (event) => {
    ...
    if (uploadedImage) {
      ...
      console.log("📸 New image selected:", uploadedImage.name);
    }
  };
  ...
  const handleFormSubmit = async (event) => {
    ...
    // checking for the empty field - only fullName is required
    if (!data.fullName) {
